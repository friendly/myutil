# Ported from C:/Dropbox/R/release_checks.R, which was itself designed to
# be copied into each package repo as .release_checks.R and sourced --
# release_preflight() already resolves the package name/version from that
# package's own DESCRIPTION/NEWS.md, so it works unmodified here as real
# myutil functions instead of a per-package copy. Originally built for
# heplots (see its .build-steps.R for the pre-2026-08-11 unstructured
# version this replaced).
#
# All calls below run with the working directory set to the *target*
# package's root (the package you're releasing), not myutil's -- that's
# unchanged from how the original script was used.
#
...
# build_readme, check, check_win_devel, revdep) -- but it's a
# checklist/tracker, not an executor. What it has that we don't:
# usethis::use_version() (the actual Version bump) and
# use_github_release()/use_dev_version() (post-acceptance housekeeping).
#
# rhub v2 (rhub::rhub_setup() + rhub::rhub_check()) -- real gap:
# release_check_win() only covers Windows (win-builder). rhub covers
# Linux/macOS/more compiler flavors CRAN itself checks against.
#
# fledge -- auto-writes NEWS.md from git commit/merge messages and bumps
# DESCRIPTION Version on bump_version(). Could address the still-manual
# "bump Version/Date, write NEWS" step, but needs disciplined bullet-style
# commit messages throughout to work well -- a workflow change, not a
# drop-in.
#
# goodpractice -- bundles rcmdcheck + covr (coverage) + lintr (style) +
# cyclocomp (complexity), none of which release_*() touches.
#
# r-lib/actions "check-standard" GitHub Actions workflow -- runs R CMD
# check continuously across OS/R-version combos on every push, complementing
# rather than replacing this script's local pre-submission gate.
#
...
#' moment. Its confirmation prompt only fires when `interactive()`; in a
#' non-interactive run (e.g. inside [release_run_all()] run in the
#' background) that check is silently skipped, so it would accept every
#' flagged word -- real typos included -- with no confirmation at all. Use
#' [release_spelling_add()] instead to add specific words you've actually
#' reviewed.
#' @export
...
#'
#' Selective and additive, unlike `spelling::update_wordlist()` (see
#' [release_spelling()]). Existing entries are kept; `words` is merged in,
#' deduplicated, and re-sorted.
#'
#' @param words character vector of words to accept, e.g. copied from
#'   [release_spelling()]'s printed output after reviewing which flagged
...
#'
#' @details
#' `devtools::build_vignettes()` *moves* (rather than copies) any
...
#' [release_cran_comments()] can build the "R CMD check results" section
...
#'
#' Fills in R CMD check results (from [release_check()]'s saved
#' `.release_check_result.rds`), the reverse dependency summary (from
#' `revdep/cran.md`, written by [release_revdep()]), and replaces the whole
#' `## Comments` section with the NEWS.md entries added since the version
#' currently on CRAN -- so it only describes what changed since CRAN last
#' saw this package, not the full changelog.
#'
...
#'   to append as a "## Known issues" section -- for things a remote check
#'   (win-builder, R-hub) flags but a local `devtools::check()` never sees,
#'   so they wouldn't otherwise show up here (e.g. URLs embedded in a
...
#' reported at the end by [release_run_all()] instead of halting things
#' partway through. Internal helper, not exported.
#'
#' @param name label for the step, used in progress/error messages
#' @param step a zero-argument function to run
#' @return `NULL` on success, or a one-line error summary string on failure
#' @keywords internal
run_step <- function(name, step) {
  ...
  tryCatch({
    ...
    cat(glue::glue("x {name} failed: {msg}\n"))
    glue::glue("{name}: {msg}")
  })
}
...
#'
#' @param full also run [release_check_win()], [release_revdep()], and
