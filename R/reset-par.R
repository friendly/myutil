...
#'
#' Returns a named list of default `par()` values, snapshotted from a fresh graphics device --
#' pass it to `par()` to actually reset the current device, e.g. `par(reset_par())`. Useful
#' after a plotting function leaves `par()` in a state you don't want to keep (e.g. `mfrow`,
#' margins), without the side effects of `dev.off()`.
#'
#' @return A named list of `par()` values, suitable for passing to `par()`
#' @export
#' @source <https://stackoverflow.com/questions/9292563/reset-the-graphical-parameters-back-to-default-values-without-use-of-dev-off>
#' @examples
#' \dontrun{
